...
//#region Unit Tests
//#region AsQueryable
test('JLinq.ReadonlyArray.AsQueryable.Test.1', function () {
    //grab only 2 items so it's easier to test
    var shorterList = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id == 1 || x.Id == 2; }).ToArray();
    //test as queryable
...
//#endregion
//#region Select Many
test('JLinq.ReadonlyArray.SelectMany.Test.1', function () {
    //how many items we should have
    var howManyItemsShouldWeHave = (UnitTestFramework._ReadonlyArrayTest.length - 2) * 2;
    //let's go grab the query and throw it into a variable
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.SelectMany(function (x) { return x.lst; });
    //push the results to an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test**** (we * 2 because we always set in our test...2 items per collection)
...
test('JLinq.ReadonlyArray.SelectMany.Test.2', function () {
    //this is a select many with a where before it
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 4; }).SelectMany(function (x) { return x.lst; });
...
test('JLinq.ReadonlyArray.SelectMany.Test.3', function () {
    //this is a select many with a where before it and then a select after it
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 4; }).SelectMany(function (x) { return x.lst; }).Select(function (x) { return { mapId: x }; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test**** (we * 2 because we always set in our test...2 items per collection)
    equal(QueryToRunResults.length, 2);
...
//#endregion
//#region Group By
test('JLinq.ReadonlyArray.GroupBy.Test.1', function () {
    //*** Group by doesn't have a lazy iterator...so we don't need to check that
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.GroupBy(function (x) { return x.GroupByKey; });
...
test('JLinq.ReadonlyArray.GroupBy.Test.2', function () {
    //*** Group by doesn't have a lazy iterator...so we don't need to check that
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.GroupBy(function (x) { return { key1: x.GroupByKey, key2: x.GroupByKey2 }; });
...
test('JLinq.ReadonlyArray.GroupBy.ChainTest.1', function () {
    //*** Group by doesn't have a lazy iterator...so we don't need to check that
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 2; }).GroupBy(function (x) { return { key1: x.GroupByKey, key2: x.GroupByKey2 }; });
...
//#endregion
//#region Where
test('JLinq.ReadonlyArray.Where.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
test('JLinq.ReadonlyArray.Where.Test.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1 || x.Id === 2; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults[0].Id, 1);
...
test('JLinq.ReadonlyArray.Where.ChainTest.1', function () {
    //go build the query
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1 || x.Id === 2; }).Take(1).ToArray();
    equal(QueryToRunResults.length, 1);
    equal(QueryToRunResults[0].Id, 1);
    equal(QueryToRunResults[0].Txt, '1');
});
test('JLinq.ReadonlyArray.Where.ChainTest.2', function () {
    //test the where clause when it's somewhere in the chain after the first call off of array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Take(5).Where(function (x) { return x.Id === 1 || x.Id === 2; }).ToArray();
    equal(QueryToRunResults.length, 2);
    equal(QueryToRunResults[0].Id, 1);
    equal(QueryToRunResults[0].Txt, '1');
    equal(QueryToRunResults[1].Id, 2);
...
test('JLinq.ReadonlyArray.Concat.TestOffOfQueryWithArray.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1; }).Concat(UnitTestFramework.BuildArray(2));
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults[0].Id, 1);
...
test('JLinq.ReadonlyArray.Concat.TestOffOfArrayWithArray.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Concat(UnitTestFramework.BuildArray(2));
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
test('JLinq.ReadonlyArray.ConcatQuery.TestOffOfQueryWithQuery.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1; }).Concat(UnitTestFramework.BuildArray(2).Where(function (x) { return x.Id === 1; }));
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults[0].Id, 1);
...
test('JLinq.ReadonlyArray.ConcatQuery.TestOffOfQueryWithQuery.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Concat(UnitTestFramework.BuildArray(2).Where(function (x) { return x.Id === 1; })).Where(function (x) { return x.Id === 4; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
test('JLinq.ReadonlyArray.ConcatQuery.TestOffOfArrayWithQuery.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Concat(UnitTestFramework.BuildArray(2).Where(function (x) { return x.Id === 1; }));
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    for (var i = 0; i < QueryToRunResults.length; i++) {
...
test('JLinq.ReadonlyArray.Union.TestOffOfQueryWithArray.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1; }).Select(function (x) { return x.Id; }).Union(UnitTestFramework.BuildArray(2).Select(function (x) { return x.Id; }).ToArray());
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
test('JLinq.ReadonlyArray.Union.TestOffOfArrayWithArray.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).ToArray().Union(UnitTestFramework.BuildArray(2).Select(function (x) { return x.Id; }).ToArray());
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    for (var i = 0; i < QueryToRunResults.length; i++) {
...
test('JLinq.ReadonlyArray.UnionQuery.TestOffOfQueryWithQuery.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1; }).Select(function (x) { return x.Id; }).Union(UnitTestFramework.BuildArray(2).Select(function (x) { return x.Id; }));
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults[0], 1);
...
test('JLinq.ReadonlyArray.UnionQuery.TestOffOfArrayWithQuery.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).ToArray().Union(UnitTestFramework.BuildArray(2).Where(function (x) { return x.Id === 1; }).Select(function (x) { return x.Id; }));
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    for (var i = 0; i < QueryToRunResults.length; i++) {
...
//#endregion
//#region Take
test('JLinq.ReadonlyArray.Take.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Take(2);
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults.length, 2);
...
test('JLinq.ReadonlyArray.Take.ChainTest.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 2; }).Take(2);
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
...
//#endregion
//#region Take While
test('JLinq.ReadonlyArray.TakeWhile.Test.1', function () {
    ...
    //go build the query
    var QueryToRun = Object.freeze([3, 3, 1, 1, 2, 3]).TakeWhile(function (x) { return x === 3 || x === 1; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
test('JLinq.ReadonlyArray.TakeWhile.ChainTest.1', function () {
    //Remember...will return all the elements before the test no longer passes. "Where" will return everything that meet the condition. TakeWhile will exit the routine wasn't it doesnt pass the expression
    //go build the query
    var QueryToRun = Object.freeze([100, 3, 3, 1, 1, 100, 2, 3]).Where(function (x) { return x !== 100; }).TakeWhile(function (x) { return x === 3 || x === 1; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults.length, 4);
...
//#endregion
//#region Skip
test('JLinq.ReadonlyArray.Skip.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Skip(1);
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults.length, 4);
...
test('JLinq.ReadonlyArray.Skip.ChainTest.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 2; }).Skip(1);
...
//#endregion
//#region Skip While
test('JLinq.ReadonlyArray.SkipWhile.Test.1', function () {
    //Remember...after predicate is met, it will return everything after that
    //go build the query
    var QueryToRun = Object.freeze([3, 3, 1, 1, 2, 3]).SkipWhile(function (x) { return x === 3; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults.length, 4);
...
test('JLinq.ReadonlyArray.SkipWhile.ChainTest.1', function () {
    //Remember...after predicate is met, it will return everything after that
    //go materialize the results into an array
    var QueryToRun = Object.freeze([100, 3, 3, 1, 1, 100, 2, 3]).Where(function (x) { return x !== 100; }).SkipWhile(function (x) { return x === 3; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults.length, 4);
...
//#endregion
//#region Aggregate
test('JLinq.ReadonlyArray.Aggregate.Test.1', function () {
    //go build the query we will use to build the query and results
    var QueryToAggregate = Object.freeze([1, 2, 3, 4]);
    //go build the query
...
test('JLinq.ReadonlyArray.Aggregate.Test.2', function () {
    //go build the query we will use to build the query and results
...
test('JLinq.ReadonlyArray.Aggregate.ChainTest.1', function () {
    //go build the query we will use to build the query and results
    var QueryToAggregate = Object.freeze([1, 2, 3, 4]);
    //go build the query
...
//#endregion
//#region Paginate
test('JLinq.ReadonlyArray.Paginate.Test.1', function () {
    //go build the query make sure we have all the records
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Paginate(1, 100);
...
test('JLinq.ReadonlyArray.Paginate.Test.1', function () {
    ...
    //go build the query make sure we have all the records
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Paginate(1, howManyRecordsPerPage);
...
test('JLinq.ReadonlyArray.Paginate.Test.3', function () {
    //just make sure we get the rest of the records on the 2nd page
    //how many records per page
    var howManyRecordsPerPage = 3;
    //go build the query make sure we have all the records
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Paginate(2, howManyRecordsPerPage);
    //materialize the array
    var results = QueryToRun.ToArray();
    //now make sure we have the correct records
...
test('JLinq.ReadonlyArray.Paginate.ChainTest.1', function () {
    //go build the query make sure we have all the records
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Paginate(1, 100);
    //go materialize the array
    var QueryToRunResults = QueryToRun.ToArray();
    //check the count
...
//#endregion
//#region First
test('JLinq.ReadonlyArray.First.Test.1', function () {
    //***First doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 1; });
    equal(QueryToRunResults.Id, 1);
    equal(QueryToRunResults.Txt, '1');
});
test('JLinq.ReadonlyArray.First.Test.2', function () {
    //***First doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 2; });
    equal(QueryToRunResults.Id, 2);
    equal(QueryToRunResults.Txt, '2');
});
test('JLinq.ReadonlyArray.First.Test.3', function () {
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 'test'; });
    }, "Can't Find First Item. Query Returned 0 Rows");
});
test('JLinq.ReadonlyArray.First.TestWithNoPredicate.1', function () {
    //run it with no predicate
    equal(true, UnitTestFramework._ReadonlyArrayTest.First() != null);
});
test('JLinq.ReadonlyArray.First.TestWithNoPredicate.2', function () {
    //run it with no predicate (should blowup)
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === -9999; }).First();
    }, "Can't Find First Item. Query Returned 0 Rows");
});
test('JLinq.ReadonlyArray.First.TestWithNoPredicate.3', function () {
    //run it with no predicate
    equal(true, [].FirstOrDefault() == null);
});
test('JLinq.ReadonlyArray.First.ChainTest.1', function () {
    //***First doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 2; }).First(function (x) { return x.Id === 3; });
    equal(QueryToRunResults.Id, 3);
});
test('JLinq.ReadonlyArray.First.ResetIteratorTest.1', function () {
    //***First doesn't have a lazy iterator. It returns the first item right away
    //Testing to make sure the iterator reset's after we call FirstOrDefault
    //build the base query
    var baseQuery = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 1; });
    //go materialize the results.
...
test('JLinq.ReadonlyArray.FirstOrDefault.Test.1', function () {
    //***First or default doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.FirstOrDefault(function (x) { return x.Id === 1; });
    equal(QueryToRunResults.Id, 1);
    equal(QueryToRunResults.Txt, '1');
});
test('JLinq.ReadonlyArray.FirstOrDefault.Test.2', function () {
    //***First or default doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.FirstOrDefault(function (x) { return x.Id === 2; });
    equal(QueryToRunResults.Id, 2);
    equal(QueryToRunResults.Txt, '2');
});
test('JLinq.ReadonlyArray.FirstOrDefault.Test.3', function () {
    //***First or default doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.FirstOrDefault(function (x) { return x.Id === 'test'; });
    equal(QueryToRunResults, null);
});
test('JLinq.ReadonlyArray.FirstOrDefault.TestWithNoPredicate.1', function () {
    //run it with no predicate
    equal(true, UnitTestFramework._ReadonlyArrayTest.FirstOrDefault() != null);
});
test('JLinq.ReadonlyArray.FirstOrDefault.TestWithNoPredicate.2', function () {
    //run it with no predicate
    equal(true, UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === -9999; }).FirstOrDefault() == null);
});
test('JLinq.ReadonlyArray.FirstOrDefault.TestWithNoPredicate.3', function () {
    //run it with no predicate
    equal(true, [].FirstOrDefault() == null);
});
test('JLinq.ReadonlyArray.FirstOrDefault.ChainTest.1', function () {
    //***First or default doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 2; }).FirstOrDefault(function (x) { return x.Id === 3; });
    equal(QueryToRunResults.Id, 3);
});
test('JLinq.ReadonlyArray.FirstOrDefault.ResetIteratorTest.1', function () {
    //***First or default doesn't have a lazy iterator. It returns the first item right away
    //Testing to make sure the iterator reset's after we call FirstOrDefault
    //build the base query
    var baseQuery = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 1; });
...
//#endregion
//#region Single
test('JLinq.ReadonlyArray.Single.Test.1', function () {
    //***Single doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Single(function (x) { return x.Id === 1; });
    equal(QueryToRunResults.Id, 1);
    equal(QueryToRunResults.Txt, '1');
});
test('JLinq.ReadonlyArray.Single.Test.2', function () {
    //***Single doesn't have a lazy iterator. It returns the first item right away
    //this test will make sure it returns null if we have 0 items that meet the predicate
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Single(function (x) { return x.Id === 200; });
    }, "Can't Find A Single Item.Query Returned 0 Rows");
});
test('JLinq.ReadonlyArray.Single.Test.3', function () {
    //***Single doesn't have a lazy iterator. It returns the first item right away
    //this test will make sure it throw's an error if we have more then 1 item
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Single(function (x) { return x.Id === 2 || x.Id === 3; });
    }, "Can't Find A Single Item.Query Returned 0 Rows");
});
test('JLinq.ReadonlyArray.Single.Test.4', function () {
    //***Single doesn't have a lazy iterator. It returns the first item right away
    //check that it chains correctly
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 2; }).Single(function (x) { return x.Id === 3; });
    equal(QueryToRunResults.Id, 3);
    equal(QueryToRunResults.Txt, '3');
});
test('JLinq.ReadonlyArray.Single.Test.5', function () {
    //***Single doesn't have a lazy iterator. It returns the first item right away
    //check that it chains correctly while having more then 1 item which should throw an error
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 2; }).Single(function (x) { return x.Id === 3 || x.Id === 4; });
...
test('JLinq.ReadonlyArray.Single.TestWithNoPredicate.1', function () {
    //run it with no predicate
    equal(true, UnitTestFramework._ReadonlyArrayTest.Take(1).Single() != null);
});
test('JLinq.ReadonlyArray.Single.TestWithNoPredicate.2', function () {
    //run it with no predicate
    throws(function () {
        //go run the method that should blow up
        [].Single();
...
test('JLinq.ReadonlyArray.Single.TestWithNoPredicate.3', function () {
    //run it with no predicate
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === -9999; }).Single();
    }, 'We Already Have A Match. Single Must Only Have 1 or 0 Items Returned. Use FirstOrDefault If You Are Expecting Multiple Items And Just Want To Grab The First Item');
});
//#endregion
//#region Single Or Default
test('JLinq.ReadonlyArray.SingleOrDefault.Test.1', function () {
    //***Single or default doesn't have a lazy iterator. It returns the first item right away
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.SingleOrDefault(function (x) { return x.Id === 1; });
    equal(QueryToRunResults.Id, 1);
    equal(QueryToRunResults.Txt, '1');
});
test('JLinq.ReadonlyArray.SingleOrDefault.Test.2', function () {
    //***Single or default doesn't have a lazy iterator. It returns the first item right away
    //this test will make sure it returns null if we have 0 items that meet the predicate
    //go materialize the results.
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.SingleOrDefault(function (x) { return x.Id === 200; });
    equal(QueryToRunResults, null);
});
test('JLinq.ReadonlyArray.SingleOrDefault.Test.3', function () {
    //***Single or default doesn't have a lazy iterator. It returns the first item right away
    //this test will make sure it throw's an error if we have more then 1 item
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.SingleOrDefault(function (x) { return x.Id === 2 || x.Id === 3; });
...
test('JLinq.ReadonlyArray.SingleOrDefault.Test.4', function () {
    //***Single or default doesn't have a lazy iterator. It returns the first item right away
    //check that it chains correctly
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 2; }).SingleOrDefault(function (x) { return x.Id === 3; });
    equal(QueryToRunResults.Id, 3);
    equal(QueryToRunResults.Txt, '3');
});
test('JLinq.ReadonlyArray.SingleOrDefault.Test.5', function () {
    //***Single or default doesn't have a lazy iterator. It returns the first item right away
    //check that it chains correctly while having more then 1 item which should throw an error
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 2; }).SingleOrDefault(function (x) { return x.Id === 3 || x.Id === 4; });
    }, 'We Already Have A Match. SingleOrDefault Must Only Have 1 or 0 Items Returned. Use FirstOrDefault If You Are Expecting Multiple Items And Just Want To Grab The First Item');
});
test('JLinq.ReadonlyArray.SingleOrDefault.TestWithNoPredicate.1', function () {
    //run it with no predicate
    equal(true, UnitTestFramework._ReadonlyArrayTest.Take(1).SingleOrDefault() != null);
});
test('JLinq.ReadonlyArray.SingleOrDefault.TestWithNoPredicate.2', function () {
    //run it with no predicate
    equal(true, [].SingleOrDefault() == null);
});
test('JLinq.ReadonlyArray.SingleOrDefault.TestWithNoPredicate.3', function () {
    //run it with no predicate
    equal(true, UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === -9999; }).SingleOrDefault() == null);
});
//#endregion
//#region Select
test('JLinq.ReadonlyArray.Select.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return { newId: x.Id, newTxt: x.Id + 1 }; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
test('JLinq.ReadonlyArray.Select.ChainTest.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 2; }).Select(function (x) { return x.Id; });
    //go materialize the results into an array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
...
//#endregion
//#region All
test('JLinq.ReadonlyArray.All.Test.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.All(function (x) { return x.Id < 100; }), true);
});
test('JLinq.ReadonlyArray.All.Test.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.All(function (x) { return x.Id === 2; }), false);
});
test('JLinq.ReadonlyArray.All.ChainTest.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 2; }).All(function (x) { return x.Id === 2; }), true);
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 2; }).All(function (x) { return x.Id === 2; }), false);
});
//#endregion
//#region Any With Null Predicate - (Depreciated) - Don't Use AnyItem, Use Any()
test('JLinq.ReadonlyArray.AnyWithNullPredicate.Test.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.Any(), true);
});
test('JLinq.ReadonlyArray.AnyWithNullPredicate.Test.2', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal([].Any(), false);
});
test('JLinq.ReadonlyArray.AnyWithNullPredicate.ResetIteratorTest.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //Testing to make sure the iterator reset's after we call FirstOrDefault
    //we need to reset the iterator after we call this method. This test method does that.
    var query = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 1; });
    //grab the count value
    equal(true, query.Any());
    //now check to make sure we have the same amount
    equal(query.Count(), 4);
});
test('JLinq.ReadonlyArray.AnyWithNullPredicate.ChainTest.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //build this off of a query then run any items(found by user)
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 2; }).Any(), true);
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 100; }).Any(), false);
});
//#endregion
//#region Any With Predicate
test('JLinq.ReadonlyArray.Any.Test.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.Any(function (x) { return x.Id > 100; }), false);
});
test('JLinq.ReadonlyArray.Any.Test.2', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.Any(function (x) { return x.Id === 2; }), true);
});
test('JLinq.ReadonlyArray.Any.ChainTest.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Any(function (x) { return x.Id === 2; }), true);
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1; }).Any(function (x) { return x.Id === 2; }), false);
});
//#endregion
//#region Last With Null Predicate
test('JLinq.ReadonlyArray.LastWithNullPredicate.Test.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Last();
    equal(4, QueryToRunResults.Id);
    equal('4', QueryToRunResults.Txt);
});
test('JLinq.ReadonlyArray.LastWithNullPredicate.Test.2', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal([].Last(), null);
});
test('JLinq.ReadonlyArray.LastWithNullPredicate.ChainTest.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //build this off of a query then run any items(found by user)
    var LastItemResult = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Last();
    equal(LastItemResult.Id, 4);
    equal(LastItemResult.Txt, '4');
    //test something where we can't find any result
    LastItemResult = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1000; }).Last();
    equal(LastItemResult, null);
});
//#endregion
//#region Last With Where
test('JLinq.ReadonlyArray.Last.Test.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Last(function (x) { return x.Id === 2; });
    equal(2, QueryToRunResults.Id);
    equal('2', QueryToRunResults.Txt);
});
test('JLinq.ReadonlyArray.Last.Test.2', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    equal(UnitTestFramework._ReadonlyArrayTest.Last(function (x) { return x.Id === 100; }), null);
});
test('JLinq.ReadonlyArray.Last.Test.3', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Last(function (x) { return x.Id === 4; });
    equal(4, QueryToRunResults.Id);
    equal('4', QueryToRunResults.Txt);
});
test('JLinq.ReadonlyArray.Last.ChainTest.1', function () {
    //*** all just returns a boolean, there is no lazy iterator
    //go materialize the results into an array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Last(function (x) { return x.Id === 4; });
    equal(4, QueryToRunResults.Id);
    equal('4', QueryToRunResults.Txt);
});
//#endregion
//#region Distinct
test('JLinq.ReadonlyArray.Distinct.Number.Test.1', function () {
    //let's build on to the default array
    var arrayToTestAgainst = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
...
test('JLinq.ReadonlyArray.Distinct.Number.Test.2', function () {
    //let's build on to the default array
    var arrayToTestAgainst = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push the new values into the array
    arrayToTestAgainst.push({ Id: 1, Txt: "100", IsActive: true, GroupByKey: "1", GroupByKey2: "1", lst: null, CreatedDate: UnitTestFramework._DateOfTest });
...
test('JLinq.ReadonlyArray.Distinct.Date.Test.3', function () {
...
test('JLinq.ReadonlyArray.Distinct.String.Test.2', function () {
    //let's build on to the default array
    var arrayToTestAgainst = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push the new values into the array
...
test('JLinq.ReadonlyArray.Distinct.ChainTest.1', function () {
    //let's build on to the default array
    var arrayToTestAgainst = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push the new values into the array
    arrayToTestAgainst.push({ Id: 1, Txt: "100", IsActive: true, GroupByKey: "1", GroupByKey2: "1", lst: null, CreatedDate: UnitTestFramework._DateOfTest });
...
//#endregion
//#region Min
test('JLinq.ReadonlyArray.Min.Test.1', function () {
    //*** Min doesn't have a lazy iterator...it just returns the min value.
    //go materialize the results into the result
    equal(UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).Min(), 0);
});
test('JLinq.ReadonlyArray.Min.Test.2', function () {
    //*** Min doesn't have a lazy iterator...it just returns the min value.
    equal([3, 2, 1].Min(), 1);
});
test('JLinq.ReadonlyArray.Min.ChainTest.1', function () {
    //*** Min doens't have a lazy iterator...it just returns the min.
    //go build the query
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Select(function (x) { return x.Id; });
    //check the min
...
//#endregion
//#region Max
test('JLinq.ReadonlyArray.Max.Test.1', function () {
    //*** Max doesn't have a lazy iterator...it just returns the max value.
    //go materialize the results into the result
    equal(UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).Max(), 4);
});
test('JLinq.ReadonlyArray.Max.Test.2', function () {
    //*** Max doesn't have a lazy iterator...it just returns the max value.
    equal([1, 2, 3].Max(), 3);
});
test('JLinq.ReadonlyArray.Max.ResetIteratorTest.1', function () {
...
//#endregion
//#region Sum
test('JLinq.ReadonlyArray.Sum.Test.1', function () {
    //*** Sum doens't have a lazy iterator...it just returns the sum.
    //go materialize the sum
    equal(UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).Sum(), 10);
});
test('JLinq.ReadonlyArray.Sum.ChainTest.1', function () {
    //*** Sum doens't have a lazy iterator...it just returns the sum.
    //go materialize the sum
    equal([1, 2, 3].Sum(), 6);
});
test('JLinq.ReadonlyArray.Sum.ResetIteratorTest.1', function () {
    //*** Sum doens't have a lazy iterator...it just returns the sum.
    //go build the query
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).Select(function (x) { return x.Id; });
    //check the sum
...
//#endregion
//#region Count
test('JLinq.ReadonlyArray.Count.Test.1', function () {
    ...
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 3; }).Count(), 2);
});
test('JLinq.ReadonlyArray.Count.Test.2', function () {
    //*** Count doens't have a lazy iterator...it just returns the count
    //* this test the count with a chain
    //this is 2 where's with a count
    //go materialize the count
    equal(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id >= 1; }).Where(function (x) { return x.Id >= 3; }).Count(), 2);
});
test('JLinq.ReadonlyArray.Count.Test.3', function () {
    //*** Count doens't have a lazy iterator...it just returns the count
    //go materialize the count
    //this is a count with a predicate off of an array
    equal(UnitTestFramework._ReadonlyArrayTest.Count(function (x) { return x.Id >= 3; }), 2);
});
test('JLinq.ReadonlyArray.Count.Test.4', function () {
    //*** Count doens't have a lazy iterator...it just returns the count
    //run count off of an array with no predicate (just a shorthand call to array.length)
    equal(UnitTestFramework._ReadonlyArrayTest.Count(), UnitTestFramework._ReadonlyArrayTest.length);
});
test('JLinq.ReadonlyArray.Count.ResetIteratorTest.1', function () {
    //*** Count doens't have a lazy iterator...it just returns the count
    //Testing to make sure the iterator reset's after we call Count
    //we need to reset the iterator after we call this method. This test method does that.
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 0; });
...
//#endregion
//#region Average
test('JLinq.ReadonlyArray.Average.Test.1', function () {
    //*** Avg doens't have a lazy iterator...it just returns the average.
    //go materialize the average
    equal(UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).Average(), 2);
});
test('JLinq.ReadonlyArray.Average.Test.2', function () {
    //*** Avg doens't have a lazy iterator...it just returns the average.
    //build the temporary array to test
    //get the average and test it
    equal([1, 2.25, 3.5].Average(), 2.25);
});
test('JLinq.ReadonlyArray.Average.ResetIteratorTest.1', function () {
...
test('JLinq.ReadonlyArray.Dictionary.Number.Test.1', function () {
    //push the list to a dictionary
    var DictionaryResult = UnitTestFramework._ReadonlyArrayTest.ToDictionary(function (x) { return x.Id; });
...
test('JLinq.ReadonlyArray.Dictionary.Date.Test.1', function () {
    //push the list to a dictionary
    var DictionaryResult = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.CreatedDate === UnitTestFramework._FirstIndexDate; }).ToDictionary(function (x) { return x.CreatedDate; });
    //going to add another dictionary so we know its instance safe
    var InstanceSafeDictionary = new ToracTechnologies.JLinq.Dictionary();
    //add an item to the instance safe dictionary
    InstanceSafeDictionary.Add(1, 1);
...
test('JLinq.ReadonlyArray.Dictionary.Boolean.Test.1', function () {
    //push the list to a dictionary
    var DictionaryResult = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.CreatedDate === UnitTestFramework._FirstIndexDate; }).ToDictionary(function (x) { return x.IsActive; });
    //going to add another dictionary so we know its instance safe
    var InstanceSafeDictionary = new ToracTechnologies.JLinq.Dictionary();
    //add an item to the instance safe dictionary
    InstanceSafeDictionary.Add(1, 1);
...
test('JLinq.ReadonlyArray.Dictionary.MultiKeyObject.Test.1', function () {
    //push the list to a dictionary
    var DictionaryResult = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.CreatedDate === UnitTestFramework._FirstIndexDate; }).ToDictionary(function (x) { return { Id: x.Id, Active: x.IsActive }; });
...
test('JLinq.ReadonlyArray.Dictionary.MultiKeyObjectWithDatePartialKey.Test.1', function () {
    //push the list to a dictionary
    var DictionaryResult = UnitTestFramework._ReadonlyArrayTest.Take(2).ToDictionary(function (x) { return { Id: x.Id, Dt: x.CreatedDate }; });
...
//#endregion
//#region HashSet
test('JLinq.ReadonlyArray.HashSet.Number.Test.1', function () {
    //just grab the ids
    var HashSetToTest = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).ToHashSet();
...
test('JLinq.ReadonlyArray.HashSet.Date.Test.1', function () {
    //declare the hashset
    var HashSetToTest = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.CreatedDate === UnitTestFramework._FirstIndexDate; }).Select(function (x) { return x.CreatedDate; }).ToHashSet();
...
test('JLinq.ReadonlyArray.HashSet.MultiKeyObject.Test.1', function () {
    //we will grab id == 2 and id == 3 to grab the specific item
    var SecondIdToTest = UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 2; });
    var ThirdIdToTest = UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 3; });
    var FourthIdToTest = UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 4; });
    //declare the hashset
    var HashSetToTest = new ToracTechnologies.JLinq.HashSet();
    //go build the hashset
    HashSetToTest.BuildHashSet(UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 2 || x.Id === 3; }));
    ...
    //make sure we don't have another item
    equal(HashSetToTest.ContainsItem(UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 4; })), false);
    //go grab all the items
    var ValuesResult = HashSetToTest.Values();
    //do we have 1 item?
    equal(ValuesResult.Count(), 2);
    ...
    //try to add the same item now
    equal(false, HashSetToTest.Add(UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 2; })));
    //try to add another item and make sure it was added
    equal(true, HashSetToTest.Add(UnitTestFramework._ReadonlyArrayTest.First(function (x) { return x.Id === 4; })));
...
test('JLinq.ReadonlyArray.HashSet.Generic.Test.1', function () {
    //just make sure a hashset works off of an array
    equal(UnitTestFramework._ReadonlyArrayTest.ToHashSet().Count(), UnitTestFramework._ReadonlyArrayTest.Count());
});
//#endregion
//#region Order By
//#region Order By Number
test('JLinq.ReadonlyArray.OrderBy.Asc.Number.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.OrderBy(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    for (var i = 0; i < QueryToRunResults.length; i++) {
...
test('JLinq.ReadonlyArray.OrderBy.Asc.Number.ChainTest.1', function () {
    //go materialize the results into the result
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).OrderBy(function (x) { return x; });
    //go materialize the array
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    for (var i = 0; i < QueryToRunResults.length; i++) {
...
test('JLinq.ReadonlyArray.OrderBy.Desc.Number.Test.1', function () {
    //go materialize the results into the result
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.OrderByDescending(function (x) { return x.Id; });
    //go materialize the array
    var QueryToRunResults = QueryToRun.ToArray();
...
test('JLinq.ReadonlyArray.OrderBy.Desc.Number.ChainTest.1', function () {
    //go materialize the results into the result
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.Id; }).OrderByDescending(function (x) { return x; });
    //go materialize the array
    var QueryToRunResults = QueryToRun.ToArray();
    //initialize to 1 so we can just subtract the length
    var IdLengthCalculator = 1;
...
test('JLinq.ReadonlyArray.OrderBy.Asc.String.Test.1', function () {
    //go build the query
...
test('JLinq.ReadonlyArray.OrderBy.Asc.String.Test.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push a new item now
    QueryToRun.push({ Id: 1000, Txt: 'abc' });
...
test('JLinq.ReadonlyArray.OrderBy.Asc.String.ChainTest.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push a new item now
    QueryToRun.splice(0, 0, { Id: 1000, Txt: 'abc' });
...
test('JLinq.ReadonlyArray.OrderBy.Desc.String.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push a new item now
    QueryToRun.push({ Id: 1000, Txt: 'abc' });
...
test('JLinq.ReadonlyArray.OrderBy.Desc.String.Test.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push a new item now
    QueryToRun.push({ Id: 1000, Txt: 'abc' });
...
test('JLinq.ReadonlyArray.OrderBy.Desc.String.ChainTest.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework.BuildArray(UnitTestFramework._DefaultItemsToBuild);
    //push a new item now
    QueryToRun.push({ Id: 1000, Txt: 'abc' });
...
test('JLinq.ReadonlyArray.OrderBy.Asc.Boolean.Test.1', function () {
    //go materialize the results into an array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.OrderBy(function (x) { return x.IsActive; });
    //go test the results
    equal(QueryToRunResults.Last().IsActive, true);
});
test('JLinq.ReadonlyArray.OrderBy.Asc.Boolean.ChainTest.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.IsActive; }).OrderBy(function (x) { return x; });
    //go test the results
    equal(QueryToRun.Last(), true);
});
test('JLinq.ReadonlyArray.OrderBy.Desc.Boolean.Test.1', function () {
    //go materialize the results into an array
    var QueryToRunResults = UnitTestFramework._ReadonlyArrayTest.OrderBy(function (x) { return x.IsActive; }).ToArray();
    //go test the results
    equal(QueryToRunResults[0].IsActive, false);
});
test('JLinq.ReadonlyArray.OrderBy.Desc.Boolean.ChainTest.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.IsActive; }).OrderBy(function (x) { return x; }).ToArray();
    //go test the results
    equal(QueryToRun[0], false);
});
//#endregion
//#region Order By Date
test('JLinq.ReadonlyArray.OrderBy.Asc.Date.Test.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.OrderBy(function (x) { return x.CreatedDate; }).ToArray();
    equal(QueryToRun[0].CreatedDate.toString(), new Date('12/1/1980').toString());
});
test('JLinq.ReadonlyArray.OrderBy.Asc.Date.ChainTest.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.CreatedDate; }).OrderBy(function (x) { return x; }).ToArray();
    equal(QueryToRun[0].toString(), new Date('12/1/1980').toString());
});
test('JLinq.ReadonlyArray.OrderBy.Desc.Date.Test.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.OrderByDescending(function (x) { return x.CreatedDate; }).ToArray();
    equal(QueryToRun.Last().CreatedDate.toString(), new Date('12/1/1980').toString());
});
test('JLinq.ReadonlyArray.OrderBy.Desc.Date.ChainTest.1', function () {
    //go materialize the results into an array
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Select(function (x) { return x.CreatedDate; }).OrderByDescending(function (x) { return x; }).ToArray();
    equal(QueryToRun.Last().toString(), new Date('12/1/1980').toString());
});
//#endregion
//#endregion
//#region Then By Order
test('JLinq.ReadonlyArray.ThenBy.Asc.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.OrderBy(function (x) { return x.Txt; }).ThenBy(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 1);
...
test('JLinq.ReadonlyArray.ThenBy.Asc.Test.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.OrderBy(function (x) { return x.Txt; }).ThenByDescending(function (x) { return x.Txt2; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 2);
...
test('JLinq.ReadonlyArray.ThenBy.Asc.Test.3', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.OrderBy(function (x) { return x.Txt; }).ThenByDescending(function (x) { return x.Txt2; }).ThenBy(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 2);
...
test('JLinq.ReadonlyArray.ThenBy.Asc.ChainTest.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.Where(function (x) { return x.Id <= 3; }).OrderBy(function (x) { return x.Txt; }).ThenBy(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 1);
...
test('JLinq.ReadonlyArray.ThenBy.Asc.ChainTest.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.Where(function (x) { return x.Id <= 3; }).OrderBy(function (x) { return x.Txt; }).ThenBy(function (x) { return x.Id; }).Select(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0], 1);
...
test('JLinq.ReadonlyArray.ThenBy.Desc.Test.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.OrderByDescending(function (x) { return x.Txt; }).ThenByDescending(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 4);
...
test('JLinq.ReadonlyArray.ThenBy.Desc.Test.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.OrderByDescending(function (x) { return x.Txt; }).ThenBy(function (x) { return x.Txt2; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 4);
...
test('JLinq.ReadonlyArray.ThenBy.Desc.Test.3', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.OrderByDescending(function (x) { return x.Txt; }).ThenBy(function (x) { return x.Txt2; }).ThenBy(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
...
test('JLinq.ReadonlyArray.ThenBy.Desc.ChainTest.1', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.Where(function (x) { return x.Id <= 3; }).OrderByDescending(function (x) { return x.Txt; }).ThenByDescending(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0].Id, 3);
...
test('JLinq.ReadonlyArray.ThenBy.Desc.ChainTest.2', function () {
    //go build the query
    var QueryToRun = UnitTestFramework._SortOrderArray.Where(function (x) { return x.Id <= 3; }).OrderByDescending(function (x) { return x.Txt; }).ThenByDescending(function (x) { return x.Id; }).Select(function (x) { return x.Id; });
    //push the results
    var QueryToRunResults = QueryToRun.ToArray();
    //go test the results
    equal(QueryToRunResults[0], 3);
...
//#endregion
//#region ElementAt
test('JLinq.ReadonlyArray.ElementAt.Test.1', function () {
    //*** ElementAt doens't have a lazy iterator...it just returns the element.
    //go build the query
    var ElementAt0 = UnitTestFramework._ReadonlyArrayTest.ElementAt(0);
    var ElementAt1 = UnitTestFramework._ReadonlyArrayTest.ElementAt(1);
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    ...
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.ElementAt(5);
    }, 'ArgumentOutOfRangeException. The size of the collection is less then the index specified. There are only ' + UnitTestFramework._ReadonlyArrayTest.length + ' elements in the query.');
});
test('JLinq.ReadonlyArray.ElementAt.ChainTest.1', function () {
    //*** ElementAt doens't have a lazy iterator...it just returns the element.
    //build the base query
    var BaseQuery = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; });
    //go build the query
    ...
    throws(function () {
        //go run the method that should blow up
        UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; }).ElementAt(5);
...
test('JLinq.ReadonlyArray.ElementAtDefault.Test.1', function () {
    //*** ElementAt doens't have a lazy iterator...it just returns the element.
    //go build the query
    var ElementAtDefault0 = UnitTestFramework._ReadonlyArrayTest.ElementAtDefault(0);
    var ElementAtDefault1 = UnitTestFramework._ReadonlyArrayTest.ElementAtDefault(1);
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    //element at 0...should be 0
    ...
    UnitTestFramework._ReadonlyArrayTest.ElementAtDefault(5);
});
test('JLinq.ReadonlyArray.ElementAtDefault.ChainTest.1', function () {
    //*** ElementAt doens't have a lazy iterator...it just returns the element.
    var BaseQuery = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id > 1; });
    //go build the query
...
//#endregion
//#region Join
test('JLinq.ReadonlyArray.Join.Test.WithOuterArray.1', function () {
    //grab the sports data
    var OuterJoinData = UnitTestFramework.BuildSports();
    //selector to join. seperating so its readable
    var JoinSelector = function (team, sport) { return { TeamDescription: team.TeamDescription, SportDescription: sport.SportDescription }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildBuildTeams().Join(OuterJoinData, function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
...
test('JLinq.ReadonlyArray.Join.ChainTest.WithOuterArray.1', function () {
    //grab the sports data
    var OuterJoinData = UnitTestFramework.BuildSports();
    //selector to join. seperating so its readable
    var JoinSelector = function (team, sport) { return { TeamDescription: team.TeamDescription, SportDescription: sport.SportDescription }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildBuildTeams().Where(function (x) { return x.SportId === 1; }).Join(OuterJoinData, function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
    //make sure we have 2 items that match
...
test('JLinq.ReadonlyArray.Join.Test.WithOuterQuery.1', function () {
    //grab the sports data
    var OuterJoinData = UnitTestFramework.BuildSports();
    //selector to join. seperating so its readable
    var JoinSelector = function (team, sport) { return { TeamDescription: team.TeamDescription, SportDescription: sport.SportDescription }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildBuildTeams().Join(OuterJoinData.Where(function (x) { return x.SportId === 1; }), function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
    //make sure we have 2 items that match
    equal(ResultOfQuery.length, 2);
...
test('JLinq.ReadonlyArray.Join.ChainTest.WithOuterQuery.1', function () {
    //grab the sports data
    var OuterJoinData = UnitTestFramework.BuildSports();
    //selector to join. seperating so its readable
    var JoinSelector = function (team, sport) { return { TeamDescription: team.TeamDescription, SportDescription: sport.SportDescription }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildBuildTeams().Where(function (x) { return x.SportId === 1 || x.SportId === 2; }).Join(OuterJoinData.Where(function (x) { return x.SportId === 1 || x.SportId === 3; }), function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
    //make sure we have 2 items that match
    equal(ResultOfQuery.length, 2);
...
//#endregion
//#region Group Join
test('JLinq.ReadonlyArray.GroupJoin.Test.WithOuterArray.1', function () {
    //selector to join. seperating so its readable
    var JoinSelector = function (sport, teams) { return { SportDescription: sport.SportDescription, TeamDescription: teams.length + ' Teams' }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildSports().GroupJoin(UnitTestFramework.BuildBuildTeams(), function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
...
test('JLinq.ReadonlyArray.GroupJoin.ChainTest.WithOuterArray.1', function () {
    //selector to join. seperating so its readable
    var JoinSelector = function (sport, teams) { return { SportDescription: sport.SportDescription, TeamDescription: teams.length + ' Teams' }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildSports().Where(function (x) { return x.SportId === 1 || x.SportId === 2; }).GroupJoin(UnitTestFramework.BuildBuildTeams(), function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
    //make sure we have 2 items that match
    equal(ResultOfQuery.length, 2);
...
test('JLinq.ReadonlyArray.GroupJoin.Test.WithOuterQuery.1', function () {
    //selector to join. seperating so its readable
    var JoinSelector = function (sport, teams) { return { SportDescription: sport.SportDescription, TeamDescription: teams.length + ' Teams' }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildSports().GroupJoin(UnitTestFramework.BuildBuildTeams().Where(function (x) { return x.SportId === 1; }), function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
    //make sure we have 2 items that match
    equal(ResultOfQuery.length, 4);
...
test('JLinq.ReadonlyArray.GroupJoin.ChainTest.WithOuterQuery.1', function () {
    //selector to join. seperating so its readable
    var JoinSelector = function (sport, teams) { return { SportDescription: sport.SportDescription, TeamDescription: teams.length + ' Teams' }; };
    //go build the base query
    var QueryToRun = UnitTestFramework.BuildSports().Where(function (x) { return x.SportId === 1 || x.SportId === 2; }).GroupJoin(UnitTestFramework.BuildBuildTeams().Where(function (x) { return x.SportId === 1; }), function (x) { return x.SportId; }, function (y) { return y.SportId; }, JoinSelector);
    //result to array
    var ResultOfQuery = QueryToRun.ToArray();
    //make sure we have 2 items that match
    equal(ResultOfQuery.length, 2);
...
//#endregion
//#region Group Join
test('JLinq.ReadonlyArray.DefaultIfEmpty.Test.1', function () {
    //grab the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === 1 || x.Id === 2; }).DefaultIfEmpty(UnitTestFramework._DefaultIfEmpty);
    //result to array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
    equal(QueryToRunResults.length, 2);
...
test('JLinq.ReadonlyArray.DefaultIfEmpty.NoResultTest.Test.1', function () {
    //grab the query
    var QueryToRun = UnitTestFramework._ReadonlyArrayTest.Where(function (x) { return x.Id === -1; }).DefaultIfEmpty(UnitTestFramework._DefaultIfEmpty);
    //result to array
    var QueryToRunResults = QueryToRun.ToArray();
    //****To-UnitTestFramework._ReadonlyArrayTest Test****
